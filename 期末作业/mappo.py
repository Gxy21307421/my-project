...
# ==================== MAPPO 模型定义 ====================
class DecentralizedActorCentralizedCriticModel(TorchModelV2, nn.Module):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        ...
        # 观测维度：每个智能体自己的观测维度（来自 obs_space）
        ...
        # 集中式Critic的输入维度：所有智能体观测的拼接
        self.centralized_obs_dim = self.local_obs_dim * self.n_agents

        print(
            f"MAPPO模型初始化: n_agents={self.n_agents}, "
            f"local_obs_dim={self.local_obs_dim}, "
            f"centralized_obs_dim={self.centralized_obs_dim}, "
        ...
        # Critic网络（集中式）：使用所有智能体的观测拼接
        critic_hidden_size = 256
        self.critic_net = nn.Sequential(
            nn.Linear(self.centralized_obs_dim, critic_hidden_size),
            nn.ReLU(),
    ...
    def _extract_global_obs(self, obs):
        # Tensor: 假设已经是全局观测
        if isinstance(obs, torch.Tensor):
            global_obs_t = obs.float()
            if global_obs_t.dim() == 1:
                global_obs_t = global_obs_t.unsqueeze(0)
            return global_obs_t

        # List / tuple: 逐个转 tensor，按最后一维拼接
        if isinstance(obs, (list, tuple)):
            tensors = []
            for o in obs:
                ...
                tensors.append(t)
            if len(tensors) > 0:
                global_obs_t = torch.cat(tensors, dim=-1)
                return global_obs_t
        ...
        # 默认：视为全局观测
        global_obs_t = torch.as_tensor(obs, dtype=torch.float32)
        if global_obs_t.dim() == 1:
            global_obs_t = global_obs_t.unsqueeze(0)
        return global_obs_t
    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        # 2. Critic 部分：全局观测
        global_obs = self._extract_global_obs(obs)
        if global_obs.dim() > 2:
            global_obs = global_obs.view(global_obs.size(0), -1)

        # 如果全局观测维度不足，简化：用本地观测重复拼成全局
        if global_obs.size(-1) < self.centralized_obs_dim:
            global_obs = local_obs.repeat(1, self.n_agents)
            global_obs = global_obs[:, : self.centralized_obs_dim]

        value = self.critic_net(global_obs).squeeze(-1)  # [B]
    ...
    def get_centralized_critic_input(self):
        return self._global_obs


# 注册 MAPPO 模型到 RLlib
ModelCatalog.register_custom_model("mappo_model", DecentralizedActorCentralizedCriticModel)
...
def train_balance_cppo_gpu():
    ...
    # 设置环境变量以使用 GPU（local_mode 下只影响设备选择，不影响逻辑）
